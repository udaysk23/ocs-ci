...
class MdBlow(object):
    ...
    def __init__(self):
        ...
        ptch = f'{{"spec": {{"resources": {{"noobaa-core": {{"limits": {{"cpu": "6","memory": "4Gi"}}}}}}}}}}'
        ptch_cmd = (
            f"oc patch storagecluster {storage_cluster} "
            f"-n {namespace}  --type merge --patch '{ptch}'"
        )
        run_cmd(ptch_cmd)
        logger.info(f"Wait for noobaa-core pod move to Running state")
    ...
    def patch_lines_in_core_pod(self):
        ...
        cmd = "NOOBAA_ROOT_SECRET=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="
        self.noobaa_core_pod.exec_cmd_on_pod(cmd)
    ...
    def monitor_db_usage(self, db_percentage):
        ...
        while not self.stop_dumping.is_set():
            ...
            if prev_db_usage == current_db_percentage:
                ...
                count = 0
            if count == 100:
                logger.error("DB Percentage is stuck from last 500 seconds to the same value")
                self.stop_dumping.set()
            current_db_percentage = self.noobaa_db_pod.exec_cmd_on_pod(
                "df -h | grep postgresql | awk '{print $5}'",
                shell=True,
