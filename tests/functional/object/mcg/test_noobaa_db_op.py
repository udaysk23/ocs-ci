...
from ocs_ci.ocs import platform_nodes, constants
...
from ocs_ci.utility.utils import get_primary_nb_db_pod, exec_cmd
...
class TestNoobaaDbOps:
    ...
    def test_db_with_large_data_before_upgrade(
        self, request, bucket_factory_session, md_blow_pre_req, mcg_obj_session, awscli_pod_session
    ):
        """
        Test noobaa upgrade after filling DB to 90%
        Script is skipping metadata comparision operation due to multiple re-execution of post-upgrade MCG operation
            ...
            2. Fill DB cluster upto 90% using MD blow script
            # 3. Capture metadata of the DB
        ...
        self.md_blow_obj = md_blow_pre_req()
        self.md_blow_obj.upload_obj_using_md_blow(bucketname, db_percentage=10)
        obj_count_pre_upgrade = len(list_objects_from_bucket(
    ...
    def test_db_with_large_data_after_upgrade(
        ...
        # Create an MCG bucket
        bucketname = request.config.cache.get("bucketname", None)
        obj_count_pre_upgrade = request.config.cache.get("object_count", None)
        obj_count_post_upgrade = len(list_objects_from_bucket(
            pod_obj=awscli_pod_session,
            s3_obj=mcg_obj_session,
            target=bucketname,
            recursive=True,
        ))
        assert obj_count_pre_upgrade == obj_count_post_upgrade, "Mismatch in object count after upgrade"
